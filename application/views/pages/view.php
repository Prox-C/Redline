      ...
      <header class="d-flex flex-wrap align-items-center justify-content-between px-4 py-4 m-0 border-bottom">
        ...
        <div class="col-1 d-flex align-items-center justify-content-end">
            ...
            if($this->session->userdata('logged_in')) {
              if($liked) {
            ?>
              <button class="btn btn-sm bg-danger rounded-circle text-align-center" onclick="window.location='<?= base_url('toggleLike/'.$car['id'])?>'"><svg class="fab-icon" xmlns="http://www.w3.org/2000/svg" height="18px" fill="currentColor" viewBox="0 0 256 256"><path d="M240,102c0,70-103.79,126.66-108.21,129a8,8,0,0,1-7.58,0C119.79,228.66,16,172,16,102A62.07,62.07,0,0,1,78,40c20.65,0,38.73,8.88,50,23.89C139.27,48.88,157.35,40,178,40A62.07,62.07,0,0,1,240,102Z"></path></svg></button>
            <?php 
              } else {
            ?>
          <button class="btn btn-sm btn-outline-danger rounded-circle text-align-center" onclick="window.location='<?= base_url('toggleLike/'.$car['id'])?>'"><svg class="fab-icon" xmlns="http://www.w3.org/2000/svg" height="18px" fill="currentColor" viewBox="0 0 256 256"><path d="M178,36c-20.09,0-37.92,7.93-50,21.56C115.92,43.93,98.09,36,78,36a66.08,66.08,0,0,0-66,66c0,72.34,105.81,130.14,110.31,132.57a12,12,0,0,0,11.38,0C138.19,232.14,244,174.34,244,102A66.08,66.08,0,0,0,178,36Zm-5.49,142.36A328.69,328.69,0,0,1,128,210.16a328.69,328.69,0,0,1-44.51-31.8C61.82,159.77,36,131.42,36,102A42,42,0,0,1,78,60c17.8,0,32.7,9.4,38.89,24.54a12,12,0,0,0,22.22,0C145.3,69.4,160.2,60,178,60a42,42,0,0,1,42,42C220,131.42,194.18,159.77,172.51,178.36Z"></path></svg></button>
            <?php 
              }
            }
            ?>
        ...
        <div class="row gap-0 p-0 border-bottom">
            ...
            <div class="col-lg-6 bg-white p-5 d-flex flex-column">
                ...
                <div class="col-lg-4 h-100 d-flex align-items-center justify-content-start">
                  ...
                  <?php if (isset($car['status']) && $car['status'] === 'Maintenance'): ?>
                      <button type="button"
                              class="btn btn-lg bg-dark-subtle fw-normal border-0 w-100 rounded-4"
                              disabled>
                          Under Maintenance
                      </button>
                  <?php else: ?>
                      <button type="button"
                              class="btn btn-lg btn-bd-primary w-100 rounded-4"
                              data-bs-toggle="modal"
                              data-bs-target="#bookModal">
                          Book now
                          <svg xmlns="http://www.w3.org/2000/svg" width="22" style="position: relative; bottom: 1px" fill="currentColor" viewBox="0 0 256 256"><path d="M224.49,136.49l-72,72a12,12,0,0,1-17-17L187,140H40a12,12,0,0,1,0-24H187L135.51,64.48a12,12,0,0,1,17-17l72,72A12,12,0,0,1,224.49,136.49Z"></path></svg>
                      </button>
                  <?php endif; ?>
                  <?php else: ?>
                  <button type="button"
                          class="btn btn-lg bg-dark-subtle fw-medium border-0 w-100 rounded-4"
                          disabled>
                      Sign in to book
                  </button>
                  <?php endif; ?>
